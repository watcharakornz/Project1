...
if (
    ...
    $stmt = $employee->updateUser($array_request);
    $db->conn = null;
    ...
    if ($stmt['status']) {
        ...
        $ret_dat['description'] = "แก้ไขข้อมูลสำเร็จ";
    } else {
        http_response_code($database->code['failed']);
        $ret_dat['status'] = "failed";
        $ret_dat['status_code'] = $database->code["failed"];
        $ret_dat['description'] = "แก้ไขข้อมูลไม่สำเร็จ เนื่องจาก: " . $stmt['execute']->errorInfo()[2];
    }
