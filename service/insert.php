...
if (
    !empty($array_request['name']) &&
    !empty($array_request['surename']) &&
    !empty($array_request['sex']) &&
    !empty($array_request['birthdtm']) &&
    !empty($array_request['email']) &&
    !empty($array_request['address']) &&
    !empty($array_request['telephone']) &&
    ...
    $stmt = $employee->insertUser($array_request);
    $db->conn = null;
    ...
    if ($stmt['status']) {
        ...
        $ret_dat['description'] = "เพิ่มข้อมูลสำเร็จ";
    } else {
        http_response_code($database->code['failed']);
        $ret_dat['status'] = "failed";
        $ret_dat['status_code'] = $database->code["failed"];
        $ret_dat['description'] = "เพิ่มข้อมูลไม่สำเร็จ เนื่องจาก: " . $stmt['execute']->errorInfo()[2];
    }
